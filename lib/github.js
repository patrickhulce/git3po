...
class GitHubApi {
  ...
  _authenticatedRequest(urlBase, queryObj = {}, method = 'get', postPayload) {
    ...
    return fetch(url, {
      ...
      console.error('Failed fetch', reason)
      return Promise.reject(new Error(reason || 'Unknown failed fetch'))
    })
  }
  ...
  fetchPullRequestsSince(date, state = 'all') {
    ...
    return this._fetchUntilFinished(responsePromise, async pullsArr => {
      for (const prItem of pullsArr) {
        ...
        // Why both? Dunno.  The pr/ payload has more data.
        //   While the issue/ has a couple extra fields (reactions, timeline_url, events_url), they don't seem wildly important.
