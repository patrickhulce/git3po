...
class GitHubApi {
  ...
  async _fetchUntilFinished(promise, acculmator) {
    const response = await promise
    const body = response.body
    const next = response.links && response.links.next
    const merged = acculmator.concat(body)
    if (next) {
      log('fetching next page - %d results so far', merged.length)
      const req = this._authenticatedRequest(next).promise()
      return this._fetchUntilFinished(req, merged)
    } else {
      log('fetched last page - %d total results', merged.length)
      return merged
    }
  }

  async fetchIssuesSince(date, state = 'all') {
