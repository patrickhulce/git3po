...
class Filter {
  ...
  static preprocess(object, issue) {
    ...
    const replacements = {
      ...
      title: issue.title,
      pull_request_number: issue.number, // eslint-disable-line camelcase
      date: relativeTimePhrase => chrono.parseDate(relativeTimePhrase),
      reviewer:
        issue.requested_reviewers &&
        issue.requested_reviewers[0] &&
        issue.requested_reviewers[0].login,
    }
  ...
  static replaceProperty(object, needle, replacement) {
    if (typeof object === 'string') {
      return object.replace(needle, replacement)
